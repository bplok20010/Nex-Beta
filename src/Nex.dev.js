	...
	Nex.define('Nex.AbstractComponent',{
		...
		configs : function(){
			return {
				...
				initRenderContentAble : true,//初始时就设置内容 否则需要自己设置
				views : {},
				plugins : [],
				defalutxType : 'panel',
				defaults : {},
		...
		initInstance : function(cfg){
			...
			if( opt.initRenderContentAble ) {
				self.initRenderContent(opt);
			}
...
(function(){
	Nex.define('Nex.Component',{
		...
		initRenderTpl : function(){
			...
			} else if( Nex.isArray(opt.renderTpl) && opt.renderTpl.length ) {
				tpl = self.tpl( tpl.join(''), renderData );	
			} else if( opt.renderTpl ) {
		...
		setContainerSize : function(width, height){
			...
			self.fireEvent('onSetContainerSize',[wh]);
			self.el.css({
				width : wh.width,
				height : wh.height	
			});
